...
ThemeColor::ThemeColor(const std::string& hex, float a)
    : hexColor(hex.empty() ? "#000000" : hex), alpha(a), red(0), green(0), blue(0) {
...
std::string ThemeColor::toHex() const {
  std::stringstream ss;
  ss << "#" << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(red * 255)
     << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(green * 255) << std::hex
     << std::setfill('0') << std::setw(2) << static_cast<int>(blue * 255);
  if (alpha != 1.0f) {
    ss << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(alpha * 255);
  }
  return ss.str();
}
