...
public:
  ...
  static std::vector<TextRange> findChangedRegions(const std::string& oldText,
...
private:
  ...
  static std::vector<TextRange> mergeRanges(const std::vector<TextRange>& ranges,
                                            size_t maxGap = 100);

  static void expandRangeToFullLines(TextRange& range, const std::string& text,
                                     size_t contextLines);
};
