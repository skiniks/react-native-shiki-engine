...
RCT_EXPORT_METHOD(findNextMatchSync : (double)scannerId text : (NSString*)
                      text startPosition : (double)startPosition resolver : (RCTPromiseResolveBlock)
                          resolve rejecter : (RCTPromiseRejectBlock)reject) {
  @try {
    ...
    NSMutableArray* captureIndices = [NSMutableArray arrayWithCapacity:result->capture_count];
...
RCT_EXPORT_METHOD(destroyScanner : (double)scannerId resolver : (RCTPromiseResolveBlock)
                      resolve rejecter : (RCTPromiseRejectBlock)reject) {
  @try {
    auto it = scanners_.find(scannerId);
    if (it != scanners_.end()) {
