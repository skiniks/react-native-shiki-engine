...
- (void)applyStyle:(const shiki::ThemeStyle &)style
  ...
  if (!style.backgroundColor.empty()) {
    UIColor *bgColor =
        [self colorFromHexString:@(style.backgroundColor.c_str())];
    if (bgColor) {
...
- (NSTimeInterval)averageStyleApplicationTime {
  return _styleApplicationCount > 0
             ? _totalStyleApplicationTime / _styleApplicationCount
             : 0;
}
...
- (UIColor *)colorFromHexString:(NSString *)hexString {
  ...
  if (hexString.length > 0) {
    ...
    if ([scanner scanHexInt:&colorCode]) {
      UIColor *color =
          [UIColor colorWithRed:((colorCode & 0xFF0000) >> 16) / 255.0
...
- (UIFont *)getFontWithStyle:(const shiki::ThemeStyle &)style
  ...
  UIFontDescriptor *newDescriptor =
      [UIFontDescriptor fontDescriptorWithFontAttributes:traits];
...
- (void)setError:(NSError **)error
    withHighlightError:(const shiki::HighlightError &)highlightError {
  if (error) {
    ...
    if (error) {
      *error =
          [NSError errorWithDomain:ShikiErrorDomain
                          ...
                          userInfo:@{
                            NSLocalizedDescriptionKey : @"Failed to parse theme"
                          }];
    }
    return NO;
  }
...
- (BOOL)setGrammarContent:(NSString *)content error:(NSError **)error {
  if (!content) {
    if (error) {
      *error = [NSError
          errorWithDomain:ShikiErrorDomain
                     code:ShikiErrorCodeInvalidInput
                 userInfo:@{
                   NSLocalizedDescriptionKey : @"Grammar content is nil"
                 }];
    }
    return NO;
  }
      ...
      if (error) {
        *error = [NSError
            errorWithDomain:ShikiErrorDomain
                   ...
                   userInfo:@{
                     NSLocalizedDescriptionKey : @"Invalid grammar format"
                   }];
      }
      return NO;
    }
      ...
      if (error) {
        *error = [NSError
            errorWithDomain:ShikiErrorDomain
                       code:ShikiErrorCodeInvalidGrammar
                   userInfo:@{
                     NSLocalizedDescriptionKey : @"Failed to parse grammar"
                   }];
      }
      return NO;
    }
...
- (void)clearCurrentTheme {
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:ShikiThemeDidChangeNotification
                    object:self];
}
    ...
    if (error) {
      *error =
          [NSError errorWithDomain:ShikiErrorDomain
                              code:ShikiErrorCodeInvalidGrammar
                          userInfo:@{
                            NSLocalizedDescriptionKey : @"No valid grammar set"
                          }];
    }
    return NO;
  }
    ...
    if (error) {
      *error = [NSError
          errorWithDomain:ShikiErrorDomain
                 ...
                 userInfo:@{
                   NSLocalizedDescriptionKey : @"Text too large to tokenize"
                 }];
    }
    return NO;
  }
...
- (NSDictionary *)attributesForStyle:(const ThemeStyle &)style {
  ...
  if (traits.count > 0) {
    UIFontDescriptor *descriptor =
        [UIFont systemFontOfSize:12.0].fontDescriptor;
    descriptor = [descriptor fontDescriptorByAddingAttributes:@{
      UIFontDescriptorTraitsAttribute : traits
    }];
    attributes[NSFontAttributeName] = [UIFont fontWithDescriptor:descriptor
                                                            size:12.0];
  }
...
- (NSAttributedString *)styledAttributedStringFromText:(NSString *)text
  ...
  for (const auto &token : tokens) {
    if (token.start + token.length <= [text length]) {
      NSRange range = NSMakeRange(token.start, token.length);
      ThemeStyle style =
          self.currentTheme->resolveStyle(token.getCombinedScope());
