...
std::vector<PlatformStyledToken>
IOSHighlighter::highlight(const std::string &code) {
  try {
    ...
    for (const auto &token : tokens) {
      PlatformStyledToken platformToken{token.start, token.length, token.style,
...
void *IOSHighlighter::createNativeView(const PlatformViewConfig &config) {
  ...
  // Set font
  UIFont *font = [UIFont fontWithName:@(config.fontFamily.c_str())
  ...
  for (const auto &token : platformTokens) {
    ...
    if (token.style.bold || token.style.italic) {
      ...
      UIFontDescriptor *descriptor =
          [baseFont.fontDescriptor fontDescriptorWithSymbolicTraits:traits];
      if (descriptor) {
        UIFont *styledFont = [UIFont fontWithDescriptor:descriptor
...
void IOSHighlighter::setViewConfig(void *view,
                                   const PlatformViewConfig &config) {
  if (!view)
    return;
...
void IOSHighlighter::handleMemoryWarning() { invalidateResources(); }
