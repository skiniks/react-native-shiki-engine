...
+ (ComponentDescriptorProvider)componentDescriptorProvider
{
    ...
    return concreteComponentDescriptorProvider<ShikiHighlighterViewComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
    NSLog(@"ShikiHighlighterView: initializing with frame %@", NSStringFromCGRect(frame));
    self = [super initWithFrame:frame];
    if (self) {
        static const auto defaultProps = std::make_shared<const ShikiHighlighterViewProps>();
        _props = defaultProps;

        self.clipsToBounds = YES;
        // TODO: temporarily match the dark background color from Dracula theme
        self.backgroundColor = [UIColor colorWithRed:40.0/255.0 green:42.0/255.0 blue:54.0/255.0 alpha:1.0];
        _fontSize = 14.0;
        _scrollEnabled = YES;
        ...
        _scrollView.contentInset = UIEdgeInsetsMake(16, 0, 16, 0); // Add top and bottom margin
        ...
        _textView.scrollEnabled = NO; // Disable text view scrolling, let scroll view handle it
...
- (void)setTokens:(NSArray *)tokens {
    NSLog(@"ShikiHighlighterView: setTokens called with %lu tokens", (unsigned long)tokens.count);
    if (_tokens != tokens) {
        _tokens = tokens;
...
- (void)setText:(NSString *)text {
    NSLog(@"ShikiHighlighterView: setText called with length %lu", (unsigned long)text.length);
    if (_text != text) {
        _text = text;
        [self updateTokensDisplay];
    }
...
- (void)setScrollEnabled:(BOOL)scrollEnabled {
    NSLog(@"ShikiHighlighterView: setScrollEnabled called with %d", scrollEnabled);
...
- (void)updateTokensDisplay {
    ...
    [attributedString addAttributes:defaultAttributes range:NSMakeRange(0, _text.length)];
    ...
    for (NSDictionary *token in _tokens) {
        ...
        if (start + length <= _text.length) {
            ...
            if ([style[@"bold"] boolValue]) {
                UIFont *boldFont = [UIFont fontWithDescriptor:[[font fontDescriptor] fontDescriptorWithSymbolicTraits:UIFontDescriptorTraitBold]
                                                       size:_fontSize];
                if (boldFont) {
            ...
            if ([style[@"italic"] boolValue]) {
                UIFont *italicFont = [UIFont fontWithDescriptor:[[font fontDescriptor] fontDescriptorWithSymbolicTraits:UIFontDescriptorTraitItalic]
                                                         size:_fontSize];
                if (italicFont) {
...
- (void)updateContentSize {
    ...
    for (NSString *line in lines) {
        CGFloat lineWidth = [line sizeWithAttributes:@{NSFontAttributeName: font}].width;
        maxWidth = MAX(maxWidth, lineWidth);
    }

    // Add padding
    maxWidth += _textView.textContainerInset.left + _textView.textContainerInset.right;
    ...
    NSLog(@"ShikiHighlighterView: Content size updated - width: %f, height: %f", maxWidth, contentHeight);
}

- (void)updateProps:(Props::Shared const &)props oldProps:(Props::Shared const &)oldProps
{
    NSLog(@"ShikiHighlighterView: updateProps called");
    const auto &newProps = *std::static_pointer_cast<const ShikiHighlighterViewProps>(props);

    NSLog(@"ShikiHighlighterView: Received props - text length: %lu, tokens count: %lu",
          newProps.text.length(),
          newProps.tokens.size());

    if (!newProps.tokens.empty()) {
        const auto& firstToken = newProps.tokens[0];
        NSLog(@"ShikiHighlighterView: First token - start: %d, length: %d, scope: %s",
              firstToken.start,
              firstToken.length,
              firstToken.scope.c_str());
    }
...
- (void)layoutSubviews
{
    ...
    NSLog(@"ShikiHighlighterView: new layout - bounds: %@", NSStringFromCGRect(self.bounds));
